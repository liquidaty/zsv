...
/* Case-insensitive value lookup for an env var, with an optional alias name.
 * Returns the (non-empty, whitespace-trimmed) value, or NULL. */
static const char *env_first(const char *name, const char *alias) {
...
int zsv_structured_resolve(const struct zsv_structured_opts *opts, enum zsv_structured_source *source) {
  ...
  if (opts && (opts->flag_json || opts->flag_toon)) {
    ...
    const char *envfmt = env_first("ZSV_STRUCTURED_FORMAT", "LQ_STRUCTURED_FORMAT");
    if (envfmt && (str_ci_eq(envfmt, "toon") || str_ci_eq(envfmt, "json"))) {
      ...
      /* agent profile */
      const char *defaults = env_first("ZSV_AGENT_DEFAULTS", "LQ_AGENT_DEFAULTS");
      int profile_on;
      if (defaults && str_ci_eq(defaults, "on"))
        ...
        profile_on = env_first("AI_AGENT", NULL) != NULL;
      if (profile_on) {
...
void zsv_structured_emit_notice(enum zsv_structured_format fmt, enum zsv_structured_source source, int quiet) {
  ...
  if (notice && str_ci_eq(notice, "off"))
    return;
  emitted = 1;
  /* e.g. "lq: TOON output (AI_AGENT); override: --json; silence: ZSV_AGENT_NOTICE=off" */
