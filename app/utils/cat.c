...
long concatenate_copy(int out_fd, int in_fd, off_t size) {
    ...
    while (total_written < size) {
        DWORD bytes_to_read = (DWORD)((size - total_written < COPY_BUFFER_SIZE) ? 
                                       (size - total_written) : COPY_BUFFER_SIZE);

        result = ReadFile(hIn, buffer, bytes_to_read, &bytes_read, NULL);
        if (!result || bytes_read == 0) { free(buffer); return -1; }

        result = WriteFile(hOut, buffer, bytes_read, &bytes_written, NULL);
        if (!result || bytes_written != bytes_read) { free(buffer); return -1; }
    ...
    while ((read_bytes = read(in_fd, buffer, COPY_BUFFER_SIZE)) > 0) {
        ...
        if (write_bytes != read_bytes) { free(buffer); return -1; }
