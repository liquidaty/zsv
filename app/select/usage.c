const char *zsv_select_usage_msg[] = {
  ...
  "  --rename <selector=newname>  : rename column(s); repeatable. <selector> is a column name (renames",
  "                                 every column with that name) or #N for the 1-based input column index",
  "                                 (the only way to target a blank-named or duplicate column). Splits on",
  "                                 the first '='. Header-only change; errors if no column matches.",
