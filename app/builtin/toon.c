...
static int main_toon(int argc, const char *argv[]) {
  ...
  static const char *const lines[] = {
    ...
    "",
    "Naming rule (derivable, no lookup table): the TOON sibling of any surface is",
    "the JSON name with the substring `json` replaced by `toon`:",
    "  --json        -> --toon            (mutually exclusive with --json)",
    "  2json         -> 2toon             (CSV -> JSON / CSV -> TOON; the command",
    "                                      name fixes the output format)",
    "  --json-object -> --toon-object",
    "  help X-json   -> help X-toon",
    "",
    "JSON <-> TOON codec (true inverses of each other):",
    "  json2toon                          JSON -> TOON",
    "  toon2json                          TOON -> JSON",
    "",
    "Grammar:",
    ...
    "",
    "This default applies only to format-unpinned commands (e.g. compare). The",
    "2json/2toon/json2toon/toon2json converters take their format from the command",
    "name and ignore both the session default and a contradicting --json/--toon.",
    "",
    ...
    "  " ZSV_USAGE_PROG " 2json data.csv | " ZSV_USAGE_PROG " json2toon  # JSON -> TOON",
