...
static char is_select_sql(const char *s) {
  ...
  if (zsv_strincmp((const unsigned char *)"select ", strlen("select "), (const unsigned char *)s, strlen("select ")))
    return 0;
  ...
  while (*pos) {
    ...
    for (const char **clause = allowed_clauses; *clause; clause++) {
      ...
      if (!zsv_strincmp((const unsigned char *)*clause, len, (const unsigned char *)pos, len)) {
        valid_token = 1;
