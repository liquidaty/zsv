...
static size_t zsvsheet_found_in_row(zsv_parser parser, size_t col_start, size_t col_count, const char *target, size_t target_len) {
  ...
  while (zsv_next_row(parser) == zsv_status_row &&
    ...
    if (zsvsheet_opts->find) { // find the next occurrence
      ...
      size_t colIndexPlus1 =
        zsvsheet_found_in_row(parser,
                              zsvsheet_opts->found_colnum,
                              col_count, zsvsheet_opts->find, find_len);
      if(colIndexPlus1) {
