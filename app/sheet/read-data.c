...
static char *ztv_found_in_row(zsv_parser parser, size_t col_count,
  ...
  return memmem(first_cell.str, last_cell.str - first_cell.str + last_cell.len,
                target, target_len);
}

static int read_data(char dest[ZTV_BUFFER_ROWS][ZTV_MAX_COLS][ZTV_MAX_CELL_LEN],
                     const char *filename,
                     struct zsv_opts *zsv_optsp,
                     size_t *max_col_countp,
                     const char *row_filter,
                     size_t start_row,
                     size_t start_col,
                     size_t header_span,
                     void *index,
                     struct ztv_opts *ztv_opts,
  ...
  zsv_parser parser = { 0 };
  if (zsv_new_with_properties(&opts, custom_prop_handler, filename, opts_used, &parser) !=
      zsv_status_ok) {
    fclose(fp);
  ...
  while (zsv_next_row(parser) == zsv_status_row &&
         rows_read < ZTV_BUFFER_ROWS) { // for each row
    original_row_num++;
    ...
    if(ztv_opts->hide_row_nums == 0) {
      ...
      else if(snprintf(dest[rows_read][0], ZTV_MAX_CELL_LEN, "%zu", original_row_num - 1) >= ZTV_MAX_CELL_LEN) // unlikely!
    ...
    for (size_t i = start_col; i < col_count && i + rownum_offset < ZTV_MAX_COLS; i++) {
      ...
      if (c.len) {
        char done = 0;
        if (c.len >= ZTV_MAX_CELL_LEN &&
            c.str[c.len - 1] >=
                128) { // we may have to truncate a multi-byte char
          int err = 0;
          size_t used_width;
          size_t bytes_to_copy =
              utf8_bytes_up_to_max_width_and_replace_newlines(
                  c.str, c.len, ZTV_MAX_CELL_LEN, &used_width, &err);
          if (!err) {
        ...
        if (!done) {
          memcpy(dest[rows_read][i+rownum_offset], c.str,
...
static void *get_data_index(struct get_data_index_data *d) {
  ...
  while (zsv_next_row(parser) == zsv_status_row) {
    if (row_count > 0 && row_filter) {
      ...
      struct zsv_cell first_cell = zsv_get_cell(parser, 0);
      struct zsv_cell last_cell = zsv_get_cell(parser, col_count - 1);
      if (!(memmem(first_cell.str,
                   last_cell.str - first_cell.str + last_cell.len, row_filter,
                   strlen(row_filter))))
        continue;
    }
    row_count++;
...
static void get_data_index_async(void **ix, const char *filename,
                                 struct zsv_opts *optsp,
                                 const char *row_filter,
                                 size_t *row_countp,
                                 struct zsv_prop_handler *custom_prop_handler,
...
static size_t ztv_find_next(const char *filename, const char *row_filter, const char *needle,
                            struct zsv_opts *zsv_opts, struct ztv_opts *ztv_opts,
                            size_t header_span,
                            struct ztv_rowcol *input_offset, struct ztv_rowcol *buff_offset, size_t cursor_row,
                            struct input_dimensions *input_dims,
                            struct zsv_prop_handler *custom_prop_handler,
                            const char *opts_used
                            ) {
  ...
  read_data(NULL, filename, zsv_opts, &input_dims->col_count,
            row_filter, input_offset->row + buff_offset->row + header_span + cursor_row - 1, 0, header_span, NULL, ztv_opts, custom_prop_handler, opts_used, NULL);
  ztv_opts->find = NULL;
