...
static size_t input_offset_centered(
...
static int ztv_goto_input_raw_row(size_t input_raw_num, size_t input_header_span,
                                  struct ztv_rowcol *input_offset,
                                  struct ztv_rowcol *buff_offset,
                                  struct input_dimensions *input_dims,
                                  size_t *cursor_rowp,
                                  struct display_dims *ddims,
  ...
  if(input_raw_num < input_offset->row // move the buffer up
    ...
    update_buffer = 1;
              ...
              && input_raw_num <= input_offset->row + buff_offset->row + (ddims->rows + ddims->header_span + ddims->footer_span))) {
...
static int cursor_right(int max_x, unsigned display_width, size_t max_col_count,
                        size_t *cursor_colp, size_t *start_colp) {
  size_t cursor_col = *cursor_colp;
  size_t start_col = *start_colp;
  int rc = 1;
  if (cursor_col + 1 < max_x / display_width &&
      start_col + cursor_col + 1 < max_col_count) {
    cursor_col++;
