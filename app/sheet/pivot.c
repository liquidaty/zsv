...
static zsvsheet_status zsv_sqlite3_to_csv(zsvsheet_proc_context_t pctx, struct zsv_sqlite3_db *zdb, const char *sql,
  ...
  if (err_msg)
    zsvsheet_set_status(pctx, "Error: %s", err_msg);
  return zst;
}
...
static zsvsheet_status zsvsheet_pivot_handler(struct zsvsheet_proc_context *ctx) {
  ...
  case zsvsheet_builtin_proc_pivot_cur_col:
    ...
    assert(expr);
    trim_len = strlen(expr);
    expr = (char *)zsv_strtrim((unsigned char *)expr, &trim_len);
    assert(expr);
    assert(trim_len <= sizeof(result_buffer) - 1);
    assert(trim_len > 0);
    expr = sqlite3_snprintf(sizeof(result_buffer), result_buffer, "\"%.*w\"", trim_len, expr);
    assert(strlen(expr) > trim_len);
    break;
  default:
    assert(0);
    return zsvsheet_status_error;
