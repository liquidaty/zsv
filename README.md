...
* Fast (maybe the fastest ever? at least where 256 SIMD operations are
  available). See [app/benchmark/README.md](app/benchmark/README.md).
...
* Includes ZSV command-line app with batteries:
  * select, count, sql query, describe, flatten, serialize, 2json, 2db, stack,
    pretty, 2tsv
* Easy to extend/customize `zsv` with a few lines of code via modular plug-in
...
* `2db`: convert from JSON to sqlite3 db. JSON must conform to the [database
  schema](docs/db.schema.json) such as is generated by `2json`
...
To build and install, you'll need a basic unix toolchain with `sh` and `make` or `gmake`:

```shell
./configure && sudo ./install.sh
```

GCC is the recommended compiler, but clang is also supported. See
