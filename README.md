...
* Fast (maybe the fastest ever? at least where 256 SIMD operations are available). See
...
* Includes ZSV command-line app with batteries:
  * select, count, sql query, describe, flatten, serialize, 2json, 2db, stack, pretty, 2tsv
...
* `2db`: convert from JSON to sqlite3 db. JSON must conform to the [database schema](docs/db.schema.json) such as is generated by `2json`
...
GCC is the recommended compiler, but clang is also supported. See [INSTALL.md](INSTALL.md) for more details.
