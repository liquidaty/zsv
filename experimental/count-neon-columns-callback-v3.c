...
column_t get_column(process_ctx_t *ctx, size_t index) {
  ...
  if (index == 0) {
    ...
    if (prev_comma_idx >= ctx->total_col_offsets ||
        (ctx->col_offsets[prev_comma_idx] & COL_OFFSET_MASK) >= ctx->row_end_offset) {
      return col;
    }
...
int main(int argc, const char *argv[]) {
  ...
  process_ctx_t ctx = {.total_rows = 0,
                       .inside_quote = 0,
