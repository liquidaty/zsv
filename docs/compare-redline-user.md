# `zsv compare --redline` — User Guide

## Overview

`zsv compare --redline` produces a self-contained JSON document that fully describes a comparison result.  A downstream tool can render an HTML or XLSX redline from this output alone, without re-reading the source CSVs.

## Flags

| Flag | Default | Description |
|------|---------|-------------|
| `--redline` | off | Emit the redline document instead of CSV or `--json` output. JSON by default; TOON with `--toon` or when the `AI_AGENT` environment default selects TOON. `--json-redline` is a legacy alias. |
...
```sh
zsv compare --redline -k loan_id a.csv b.csv
```
...
```sh
zsv compare --redline -k loan_id --include-unchanged-rows a.csv b.csv
```
...
```sh
zsv compare --redline -k loan_id --tolerance 0.01 a.csv b.csv
```
...
```sh
zsv compare --redline -k loan_id --tolerance 0.01 --include-tolerated a.csv b.csv
```
...
```sh
zsv compare --redline -k id a.csv b.csv c.csv
```
